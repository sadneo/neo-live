...
ro plugin, then rw plugin, then crdt
...
- [ ] Editor Plugin (rw)
- plugin spawns both server and client and connects to the server in one go
- crdt implementation on server
	-  need to diff client changes then send operations to server
	- server resolves then broadcasts
